...
class MentorIdentityScreen extends StatefulWidget {
  final SignUpDataNew signUpData;

  const MentorIdentityScreen({
    super.key,
    required this.signUpData,
  });

  @override
  State<MentorIdentityScreen> createState() => _MentorIdentityScreenState();
}

class _MentorIdentityScreenState extends State<MentorIdentityScreen> {
  ...
  void initState() {
    super.initState();
    if (widget.signUpData.mentorGender != null) {
      if (_genderOptions.contains(widget.signUpData.mentorGender)) {
        _selectedGenderOption = widget.signUpData.mentorGender;
      } else {
        _selectedGenderOption = 'Self-describe';
        _genderController.text = widget.signUpData.mentorGender!;
      }
    }
    if (widget.signUpData.mentorColorRace != null) {
      if (_colorRaceOptions.contains(widget.signUpData.mentorColorRace)) {
        _selectedColorRaceOption = widget.signUpData.mentorColorRace;
      } else {
        _selectedColorRaceOption = 'Self-describe';
        _colorRaceController.text = widget.signUpData.mentorColorRace!;
      }
    }
    if (widget.signUpData.mentorIsDisabled != null) {
      _selectedDisabilityOption =
          widget.signUpData.mentorIsDisabled! ? 'Yes' : 'No';
    }
  }
          ...
          onChanged: (value) {
            setState(() {
              ...
              if (value != 'Self-describe') {
                widget.signUpData.mentorGender = value;
        ...
        DropdownButtonFormField<String>(
          ...
          onChanged: (value) {
            setState(() {
              ...
              _colorRaceError = null;
              if (value != 'Self-describe') {
                widget.signUpData.mentorColorRace = value;
        ...
        DropdownButtonFormField<String>(
          ...
          onChanged: (value) {
            setState(() {
              ...
              if (value == 'Yes') {
                widget.signUpData.mentorIsDisabled = true;
              } else if (value == 'No') {
                widget.signUpData.mentorIsDisabled = false;
              } else {
                widget.signUpData.mentorIsDisabled = null;
              }
            });
          },
        ),
